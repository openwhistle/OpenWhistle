...
def generate_report_pdf(report: Report) -> bytes:
    ...
    if report.notes:
        ...
        for note, note_text in zip(report.notes, decrypt_note_contents(report), strict=True):
            ...
            pdf.cell(
                0,
                5,
                f"{_safe(note.author_username)}  ·  {_fmt_dt(note.created_at)}",
                new_x=XPos.LMARGIN,
                new_y=YPos.NEXT,
            )
            pdf.set_font("Helvetica", "", 10)
    ...
    footer_text = (
        "This document was generated by OpenWhistle - confidential, for authorised use only."
    )
