...
# Helvetica's core font is WinAnsi (~latin-1): typographic punctuation a
# whistleblower's own text editor added has a plain-ASCII equivalent, so it
# is not worth losing to "?" as the errors="replace" fallback below would.
...
def _safe(text: str) -> str:
    return text.translate(_PUNCTUATION).encode("latin-1", errors="replace").decode("latin-1")
