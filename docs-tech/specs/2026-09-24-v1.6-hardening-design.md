...
Design and responsiveness (package C) are **not** in this release; they move to v1.7 in `ROADMAP.md`.
...
| D3 | `docs-tech/threat-model.md`: attackers, assets, defended, **not** defended (one `SECRET_KEY` for JWT and encryption, no rotation; operator TLS) | — |
...
| Item | Why not now |
|---|---|
| Separate keys for JWT and encryption, rotation | needs a data migration; own release. D3 records it |
| Admin timestamps rounded to the day | changes the SLA display; needs its own decision |
| TLS on by default in the bundled nginx | needs certificates; stays an operator step, docs point to it |
| ldap3/boto3 as optional extras | the official image needs both; saves nothing |
| Design and responsiveness | v1.7 |
