...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "role", sa.Enum("superadmin", "admin", "case_manager", name="adminrole"), nullable=False
        ),
    ...
    op.create_index(
        op.f("ix_report_categories_org_id"), "report_categories", ["org_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("received", "in_review", "pending_feedback", "closed", name="reportstatus"),
            nullable=False,
        ),
        sa.Column(
            "submission_mode",
            sa.Enum("anonymous", "confidential", name="submissionmode"),
    ...
    op.create_index(
        op.f("ix_report_messages_report_id"), "report_messages", ["report_id"], unique=False
    )
    ...
    op.execute(
        sa.text(
            ...
            "VALUES (CAST(:org_id AS uuid), 'Default Organisation', 'default', true, NOW())"
    ...
    op.execute(
        ...
        VALUES
            (gen_random_uuid(), 'financial_fraud',  'Financial Fraud',
                'Finanzbetrug', TRUE, TRUE,  1, CAST(:org_id AS uuid)),
            (gen_random_uuid(), 'workplace_safety', 'Workplace Safety',
                'Arbeitssicherheit', TRUE, TRUE,  2, CAST(:org_id AS uuid)),
            (gen_random_uuid(), 'environmental',    'Environmental Violation',
                'Umweltverstoss', TRUE, TRUE,  3, CAST(:org_id AS uuid)),
            (gen_random_uuid(), 'corruption',       'Corruption / Bribery',
                'Korruption / Bestechung', TRUE, TRUE,  4, CAST(:org_id AS uuid)),
            (gen_random_uuid(), 'data_protection',  'Data Protection Violation',
                'Datenschutzverstoss', TRUE, TRUE,  5, CAST(:org_id AS uuid)),
            (gen_random_uuid(), 'discrimination',   'Discrimination / Harassment',
                'Diskriminierung / Belästigung', TRUE, TRUE, 6, CAST(:org_id AS uuid)),
            (gen_random_uuid(), 'other',            'Other',
                'Sonstiges', TRUE, TRUE, 99, CAST(:org_id AS uuid))
    """
        ).bindparams(org_id=_DEFAULT_ORG_ID)
    )
